...
int maze[MSZ][MSZ] = {0};
extern bool bfs_is_running = false;
extern bool dfs_is_running = false;
extern bool uc_is_running = false;
extern bool best_first_search_is_running = false;
extern bool a_star_search_is_running = false;
...
double ManhattanDistance(int index1, int index2)
{
	double x1, y1, x2, y2;
...
void InitGraph()
{
	...
	// compute h 
	if (best_first_search_is_running)
	{
		for (i = 0; i < NUM_NODES; i++)
		{
			nodes[i].setH(double(Distance(i, 1)));
		}
	}
	if (a_star_search_is_running)
	{
		for (i = 0; i < NUM_NODES; i++)
		{
			nodes[i].setH(double(ManhattanDistance(i, 1)));
		}
	}
...
void CheckNeighborInGraphAStar(Node* pCurrent, int neighbor_index,double cost)
{
	switch (nodes[neighbor_index].getColor())
	{
	case WHITE:
		...
		nodes[neighbor_index].setParent(pCurrent);
		nodes[neighbor_index].setG(pCurrent->getG() + cost);
		nodes[neighbor_index].setF(nodes[neighbor_index].getG()+ nodes[neighbor_index].getH());
		pq.push(&nodes[neighbor_index]);
		break;
	case GRAY:
		break;
...
void CheckNeighborInGraphBest(Node* pCurrent, int neighbor_index)
{
	switch (nodes[neighbor_index].getColor())
	{
	case WHITE:
...
void AStarSearchIteration()
{
	...
	{
		cout << "There is no solution\n";
		a_star_search_is_running = false;
	}
	else // pq is not empty
	{
		...
		for (auto it : pCurrent->GetOutgoing()) // it is iterator on (outgoing) edges
		{
			...
			if (nodes[neighbor_index].getColor() != BLACK && nodes[neighbor_index].getColor() != START)
				CheckNeighborInGraphAStar(pCurrent, neighbor_index,it.getCost());
		}

	}
}
void BFirstSearchIteration()
{
	Node* pCurrent;
	int neighbor_index;
	...
	{
		cout << "There is no solution\n";
		best_first_search_is_running = false;
	}
	else // pq is not empty
	{
		pCurrent = pq.top(); // pq.top is the "best" (defined in CompareNodes: G) Node 
		...
		for (auto it : pCurrent->GetOutgoing()) // it is iterator on (outgoing) edges
		{
			...
			if (nodes[neighbor_index].getColor() != BLACK && nodes[neighbor_index].getColor() != START)
				CheckNeighborInGraphBest(pCurrent, neighbor_index);
		}
...
void idle()
{
	...
	if (uc_is_running)
		UCIteration();
	if (best_first_search_is_running)
		BFirstSearchIteration();
	if (a_star_search_is_running)
		AStarSearchIteration();
...
void menu(int choice)
{
	...
	case 4:  //Best First Search
		best_first_search_is_running = true;
		break;
	case 5:  //A* Search
		a_star_search_is_running = true;
		break;
	}
}
...
// add menu
	...
	glutAddMenuEntry("Run Best First Search", 4);
	glutAddMenuEntry("Run A* Search", 5);
